...
// import { ChartJSNodeCanvas } from "chartjs-node-canvas"; // no compatible con Vercel
...
reportesRouter.get("/calificaciones/pdf", async (req, res) => {
  try {
    ...
    // Charts deshabilitados (no compatible con Vercel — usar Render para habilitar)
...
Datos diarios:\n${resumenIA}`,
    ...
    const colW = CW / 4;
    doc.roundedRect(PAD, Y, CW, 18).fill("#f1f5f9");
    ["FECHA","ATENCIONES","PUNTUALIDAD","TRATO / RESOLUCIÓN"].forEach((h, i) => {
      doc.fillColor("#1a3a6b").fontSize(7).font("Helvetica-Bold")
        .text(h, PAD + i * colW + 4, Y + 5, { width: colW - 8 });
    });
    Y += 18;
    rows.slice(0, 10).forEach((r, idx) => {
      const bg = idx % 2 === 0 ? "white" : "#f8fafc";
      doc.roundedRect(PAD, Y, CW, 15).fill(bg);
      const cols = [
        String(r.dia).slice(0, 10),
        String(r.total),
        Number(r.puntualidad).toFixed(2),
        `${Number(r.trato).toFixed(2)} / ${Number(r.resolucion).toFixed(2)}`
      ];
      cols.forEach((v, i) => {
        doc.fillColor("#374151").fontSize(7.5).font("Helvetica")
          .text(v, PAD + i * colW + 4, Y + 4, { width: colW - 8 });
      });
      Y += 15;
    });

    doc.roundedRect(PAD + CW - sideW, Y + 6, sideW, chartH - 30, 5).fill("#eff6ff");
    doc.roundedRect(PAD + CW - sideW, Y + 6, sideW, chartH - 30, 5).lineWidth(0.8).strokeColor("#bfdbfe").stroke();
    doc.fillColor("#1e40af").fontSize(8).font("Helvetica-Bold")
      .text("DÍA CON MÁS\nATENCIONES", PAD + CW - sideW + 6, Y + 16, { width: sideW - 12, align: "center", lineGap: 2 });
    doc.fillColor("#1d4ed8").fontSize(9).font("Helvetica-Bold")
      .text(String(topDay.dia).slice(0, 10), PAD + CW - sideW + 6, Y + 50, { width: sideW - 12, align: "center" });
    doc.fillColor("#1a3a6b").fontSize(22).font("Helvetica-Bold")
      .text(String(topDay.total), PAD + CW - sideW + 6, Y + 66, { width: sideW - 12, align: "center" });
    Y += chartH + 4;

    // SECTION 3 — OBSERVACIÓN
    sectionHeader(doc, PAD, Y, 3, "OBSERVACIÓN DEL PERÍODO", W - PAD);
    Y += 26;
    doc.roundedRect(PAD, Y, CW, 50, 5).fill("#fefce8");
    doc.roundedRect(PAD, Y, CW, 50, 5).lineWidth(0.8).strokeColor("#fde68a").stroke();
    doc.fillColor("#78350f").fontSize(8).font("Helvetica")
      .text(ia.observacion, PAD + 10, Y + 10, { width: CW - 20, lineGap: 2 });
    Y += 64;
